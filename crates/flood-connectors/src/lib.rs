...
impl SourceCatalogRequest {
    pub fn validate(&self) -> Result<(), ConnectorOperationError> {
        if !(1..=100).contains(&self.limit) {
            return Err(ConnectorOperationError::InvalidRequest(
                "source page limit must be between 1 and 100".into(),
            ));
        }
        if self.query.as_ref().is_some_and(|value| value.len() > 300)
            || self.cursor.as_ref().is_some_and(|value| value.len() > 500)
        {
            return Err(ConnectorOperationError::InvalidRequest(
                "query or cursor exceeds bounded connector limits".into(),
            ));
        }
        Ok(())
    }
...
impl TimelineRequest {
    pub fn validate(&self) -> Result<(), ConnectorOperationError> {
        if !(1..=100).contains(&self.limit) {
            return Err(ConnectorOperationError::InvalidRequest(
                "timeline page limit must be between 1 and 100".into(),
            ));
        }
        if self.source_id.trim().is_empty()
            || self.source_id.len() > 300
            || self.cursor.as_ref().is_some_and(|value| value.len() > 500)
        {
            return Err(ConnectorOperationError::InvalidRequest(
                "source id or cursor is invalid".into(),
            ));
        }
        Ok(())
    }
...
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
pub struct ConnectorProjectBinding {
...
impl ConnectorProjectBinding {
    pub fn validate(
        &self,
        descriptor: &ConnectorDescriptor,
    ) -> Result<(), ConnectorOperationError> {
        if !descriptor.supports_project_binding || descriptor.id != self.connector_id {
            return Err(ConnectorOperationError::OutOfScope(
                "connector is not bound to this project source".into(),
            ));
        }
        ...
        {
            return Err(ConnectorOperationError::InvalidRequest(
                "project binding is invalid".into(),
            ));
        }
        Ok(())
    }
...
pub enum ConnectorOperationError {
    ...
    NotReady(String),
    #[error("connector authentication is stale: {0}")]
    AuthExpired(String),
    #[error("connector permission denied: {0}")]
    PermissionDenied(String),
    #[error("connector source is outside the project binding: {0}")]
    OutOfScope(String),
    #[error("connector rate limit reached; retry after {retry_after_seconds:?} seconds: {message}")]
    RateLimited {
...
mod tests {
    ...
    fn ready<T>(mut future: ConnectorFuture<'_, T>) -> Result<T, ConnectorOperationError> {
        match future
            .as_mut()
            .poll(&mut Context::from_waker(Waker::noop()))
        {
            Poll::Ready(value) => value,
            Poll::Pending => panic!("fixture connector must resolve synchronously"),
        }
    }

    struct FixtureConnector;

    impl ConnectorIdentity for FixtureConnector {
        fn descriptor(&self) -> ConnectorDescriptor {
            ConnectorDescriptor::new(
                "fixture",
                "Fixture",
    ...
    impl ConnectorAdapter for FixtureConnector {
            ...
            Box::pin(async move {
                ...
                Ok(SourcePage {
                    sources: vec![ConnectorSource {
                        ...
                        kind: ConnectorSourceKind::Other,
                        label: "Source".into(),
                        detail: None,
                        url: None,
                        agent_access: false,
    ...
    #[test]
    fn fixture_connector_passes_bounded_scope_contract() {
        let connector = FixtureConnector;
        let descriptor = connector.descriptor();
        descriptor.validate().unwrap();
        assert_eq!(connector.status().health, ConnectorHealth::Ready);
        ...
        binding.validate(&descriptor).unwrap();
        let page = ready(connector.list_sources(SourceCatalogRequest {
            query: None,
            cursor: None,
            limit: 10,
        }))
        ...
        let batch = ready(connector.read_timeline(TimelineRequest {
            source_id: binding.source_id,
            cursor: None,
            limit: 25,
        }))
        .unwrap();
        assert_eq!(batch.connector_id, descriptor.id);
        assert!(!batch.has_more);

        assert!(matches!(
            ready(connector.list_sources(SourceCatalogRequest {
                query: None,
                cursor: None,
                limit: 101,
            })),
            Err(ConnectorOperationError::InvalidRequest(_))
        ));
        assert!(matches!(
            ConnectorProjectBinding {
                project_id: "project-1".into(),
                connector_id: "other".into(),
                source_id: "source:1".into(),
                agent_access: true,
            }
            .validate(&descriptor),
            Err(ConnectorOperationError::OutOfScope(_))
        ));
    }
}
