...
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq, PartialOrd, Ord)]
pub struct MutationCompensatingAction {
    pub kind: String,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub provider: Option<String>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
pub struct MutationPlanDraft {
    ...
    pub compensating_actions: Vec<MutationCompensatingAction>,
...
pub enum MutationPlanError {
    ...
    #[error("mutation compensating action kind must be non-empty and trimmed")]
    InvalidCompensatingAction,
...
impl MutationPlan {
    pub fn new(draft: MutationPlanDraft) -> Result<Self, MutationPlanError> {
        ...
        Ok(Self {
            ...
            compensating_actions: draft.compensating_actions,
    ...
    fn as_draft(&self) -> MutationPlanDraft {
        MutationPlanDraft {
            ...
            compensating_actions: self.compensating_actions.clone(),
...
fn validate_draft(draft: &MutationPlanDraft) -> Result<(), MutationPlanError> {
    ...
    if draft.compensating_actions.iter().any(|action| {
        action.kind.trim().is_empty()
            || action.kind.trim() != action.kind
            || action
                .provider
                .as_deref()
                .is_some_and(|provider| provider.trim().is_empty() || provider.trim() != provider)
    }) {
        return Err(MutationPlanError::InvalidCompensatingAction);
    }
    Ok(())
}
...
struct DigestContent {
    ...
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    compensating_actions: Vec<MutationCompensatingAction>,
...
fn digest_draft(draft: &MutationPlanDraft) -> Result<String, MutationPlanError> {
    ...
    let operations = draft
        ...
        .collect();
    let compensating_actions = draft
...
mod tests {
    ...
    #[test]
    fn compensating_actions_are_content_bound_without_changing_empty_legacy_digest_shape() {
        let base = MutationPlan::new(draft(json!({"status": "completed"}), "v1")).unwrap();
        let mut compensated = draft(json!({"status": "completed"}), "v1");
        compensated.external_effect = MutationExternalEffect::Write;
        compensated.reversibility = MutationReversibility::BestEffort;
        compensated.compensating_actions = vec![MutationCompensatingAction {
            kind: "delete_remote_artifact".into(),
            provider: Some("example".into()),
            target: Some(MutationEntityRef {
                kind: "remote_artifact".into(),
                id: "remote-1".into(),
                project_id: None,
            }),
        }];
        let compensated = MutationPlan::new(compensated).unwrap();

        assert_ne!(base.content_digest, compensated.content_digest);
        compensated.verify_integrity().unwrap();
        assert_eq!(compensated.compensating_actions.len(), 1);
    }
