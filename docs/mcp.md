...
By default, flood.md synchronizes and groups inbox signals locally but does not send them to a model. Under **Settings → MCP and AI**, the user can select an installed Codex, Claude Code, or Gemini CLI, keep automatic local selection, or explicitly configure Jev as an optional cloud decision engine. `Auto` never selects Jev and no connector content leaves the device until the user saves a Jev key, selects Jev, and enables background processing. One explicit switch allows the desktop app to send bounded batches to the selected engine, create only clear tasks, link duplicates, and skip noise. Events wait for up to 45 seconds and are grouped into batches of up to 12, so each new message does not launch a separate model call. Enabling it also registers Windows autostart; closing the window hides flood.md in the system tray, while “Quit” in the tray menu stops the process completely. Ordinary triage needs no per-project setup: linked Telegram chats and their inbox modes already define the scope. See [Jev automation](./jev-automation.md) for the adapter and privacy boundary.
...
Call <code>get_project_brief</code> before planning or changing a specific project and provide the concrete <code>intent</code>, <code>purpose</code>, and affected <code>target_paths</code>. It returns the canonical <code>work_packet</code>: project statement, current memory, permitted documents, rules, skills, sources, and open tasks. Rules with Agent access are persistent project guidance; skills are selected from the objective and paths even before a dedicated task exists. No project item expands permissions or replaces previews, expected versions, or user confirmation.
...
The brief also returns a <code>context_revision</code> and records a receipt for the current MCP session. Project and task mutations without that receipt are rejected. If a mandatory rule or selected skill does not fit in full, <code>budget.truncations</code> and <code>check_project_context</code> expose its ID, and the receipt blocks mutations until the agent reads every listed material through <code>get_project_workspace_item</code>. If another process changes <code>project.md</code>, memory, an enabled rule, or a skill, the receipt becomes stale and the server requires another <code>get_project_brief</code> or <code>get_task_work_context</code> call. A version-checked mutation made by the same MCP session advances only the project or material version that the server actually wrote; concurrent unseen changes and unread guidance remain stale/incomplete. Read-only tools remain available without a receipt.

The server also exposes permitted context through standard <code>resources/list</code>, <code>resources/templates/list</code>, and <code>resources/read</code>. <code>flood://projects/{project_id}/context</code> returns the project statement and active memory; document, rule, and skill URIs are available only with Agent access. A full resource read of a mandatory rule or selected skill satisfies the same session receipt.
...
The desktop screen **Settings → MCP and AI** shows runtime compatibility, storage health, a bounded metadata-only activity log, and an isolated self-check. Provenance rows expose the initiator/provider, short approved-plan digest, operation count, and recovery availability; task bodies, message text, mutation payloads, credentials, and secrets are not written to that audit record.
