...
fn context_check_and_lists_do_not_acknowledge_incomplete_rules() {
    ...
    server
        .mark_project_material_read(&project.id, &rule)
        .unwrap();
    assert_eq!(check(&server, &project).status, "incomplete");
    read(&server, &updated, false);
    assert_eq!(check(&server, &project).status, "current");
...
#[test]
fn routed_skill_must_be_fully_read_when_packet_truncates_it() {
    let (server, project) = setup();
    let skill = server
        .store
        .create_project_workspace_item_idempotent(
            ...
            ProjectWorkspaceItemKind::Skill,
            "Аудит интерфейса",
            Some("UI типографика и компоненты"),
            &"Проверить визуальную систему интерфейса\n".repeat(500),
            true,
            "pending-routed-skill",
        )
        .unwrap()
        .value;
    let (packet, evidence) = ContextBuilder::new(&server.store)
        .with_intent("Провести аудит UI интерфейса")
        .with_char_budget(8_000)
        .for_project_with_evidence(&project.id, WorkPurpose::Execute, vec![], vec![])
        .unwrap();
    ...
    let incomplete = check(&server, &project);
    assert_eq!(incomplete.status, "incomplete");
    assert_eq!(incomplete.pending_skill_ids, vec![skill.id.clone()]);
    assert!(server.require_project_context(&project.id).is_err());

    read(&server, &skill, false);
    let current = check(&server, &project);
    assert_eq!(current.status, "current");
    assert!(current.pending_skill_ids.is_empty());
    server.require_project_context(&project.id).unwrap();
}
