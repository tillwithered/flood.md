...
`--manifest` returns the server version, negotiated protocol version, tool count, and a stable semantic revision of the full tool catalog. The same revision is available through `get_runtime_info` and appears in Settings. The protocol test opens a clean stdio session, completes `initialize` → `initialized` → every `tools/list` page, and requires its tool count and SHA-256 fingerprint to exactly match runtime and manifest. If the binary and an already connected session report different revisions, or the session does not report one yet, restart the MCP client or open a new session: a static stdio server's catalog is fixed when its process starts, so it does not declare `listChanged`.
