use crate::{
    ...
    InboxCandidateStatus, MessageSnapshot, NewProjectKnowledgeProposal, Project,
    ProjectAutomationPolicy, ProjectKnowledgeProposal, ProjectKnowledgeProposalPayload,
    ProjectKnowledgeProposalState, ProjectKnowledgeProposalTarget, ProjectResource,
    ...
    TaskBatchOperationResult, TaskBatchOutcome, TaskBatchReference, TaskBatchRollbackOutcome,
...
const MAX_TASK_BATCH_RECEIPT_BYTES: u64 = 64 * 1024 * 1024;
...
pub struct ProjectExportManifest {
    ...
    pub files: Vec<ProjectExportFile>,
}

#[derive(Clone, Debug, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
pub struct ProjectImportConflict {
    pub kind: String,
    pub id: String,
    pub reason: String,
}
...
pub struct ProjectImportReport {
    ...
    pub conflicts: Vec<ProjectImportConflict>,
}

#[derive(Clone, Debug, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
pub struct DiagnosticAgentRun {
...
pub struct SanitizedDiagnosticReport {
    ...
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub recent_agent_runs: Vec<DiagnosticAgentRun>,
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub recent_activity: Vec<DiagnosticActivity>,
}
...
struct TaskBatchReceipt {
    ...
    tasks: Vec<Task>,
    #[serde(default)]
    artifacts: Vec<TaskBatchArtifactSnapshot>,
}

#[derive(Clone, Debug, Serialize, Deserialize)]
struct TaskBatchArtifactSnapshot {
    project_id: String,
    task_id: String,
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
enum TaskBatchWriteBoundary {
    JournalPrepared,
    TaskWritten,
    JournalCommitted,
}

enum TaskBatchRestoreAction {
...
fn rename_with_transient_retry(source: &Path, destination: &Path) -> std::io::Result<()> {
    ...
    unreachable!("bounded rename retry always returns from the loop")
}
...
impl Store {
    ...
    pub fn sanitized_diagnostic_report(&self) -> SanitizedDiagnosticReport {
        let diagnostics = self.diagnostics();
        let mut recent_agent_runs = self
            .list_agent_runs(false)
            .unwrap_or_default()
            .into_iter()
            .take(25)
            .map(|run| DiagnosticAgentRun {
                correlation_id: run.id,
                provider: run.provider,
                state: run.state,
                turn_count: run.turns.len(),
                updated_at: run.updated_at,
            })
            .collect::<Vec<_>>();
        recent_agent_runs.sort_by_key(|run| Reverse(run.updated_at));
        let recent_activity = self
            .list_activity(None, 50)
            .map(|page| {
                page.events
                    .into_iter()
                    .map(|event| DiagnosticActivity {
                        correlation_id: event
                            .provenance
                            .as_ref()
                            .and_then(|value| value.run_id.clone())
                            .or_else(|| {
                                event.provenance.as_ref().and_then(|value| {
                                    (!value.approved_plan_id.is_empty())
                                        .then(|| value.approved_plan_id.clone())
                                })
                            })
                            .unwrap_or(event.id),
        ...
        SanitizedDiagnosticReport {
            ...
            installation_fingerprint: digest(self.root.to_string_lossy().as_bytes())[..16]
                .to_owned(),
            format_version: diagnostics.format_version,
            healthy: diagnostics.healthy,
            project_count: diagnostics.project_count,
            open_task_count: diagnostics.open_task_count,
            completed_task_count: diagnostics.completed_task_count,
            trashed_task_count: diagnostics.trashed_task_count,
            pending_automation_event_count: diagnostics.pending_automation_event_count,
            failed_automation_event_count: diagnostics.failed_automation_event_count,
            issue_codes: (0..diagnostics.issues.len())
                .map(|index| format!("store_check_{}", index + 1))
                .collect(),
            recent_agent_runs,
            recent_activity,
        }
    ...
    pub fn export_sanitized_diagnostics(
        &self,
        destination: &Path,
    ) -> Result<SanitizedDiagnosticReport, StoreError> {
        let report = self.sanitized_diagnostic_report();
    ...
    pub fn create_project_knowledge_proposal(
        &self,
        draft: NewProjectKnowledgeProposal,
    ) -> Result<ProjectKnowledgeProposal, StoreError> {
        self.create_project_knowledge_proposal_with_run(draft, None)
    }

    pub fn create_project_knowledge_proposal_for_run(
        &self,
        draft: NewProjectKnowledgeProposal,
        run_id: &str,
    ) -> Result<ProjectKnowledgeProposal, StoreError> {
        ...
        if run.project_id != draft.project_id {
            return Err(StoreError::Validation(
                "запуск агента принадлежит другому проекту".into(),
            ));
        }
        if !matches!(
            run.state,
            AgentRunState::ReadyForReview | AgentRunState::Accepted
        ) {
            return Err(StoreError::Validation(
                "предложение можно связать только с готовым результатом агента".into(),
            ));
        }
        if run.turns.is_empty() {
            return Err(StoreError::Validation(
                "у запуска нет неизменяемого receipt контекста".into(),
            ));
        }
        self.create_project_knowledge_proposal_with_run(draft, Some(run_id.to_owned()))
    }

    fn create_project_knowledge_proposal_with_run(
        &self,
        draft: NewProjectKnowledgeProposal,
        source_run_id: Option<String>,
    ) -> Result<ProjectKnowledgeProposal, StoreError> {
        let NewProjectKnowledgeProposal {
            project_id,
            target,
            base_version,
            payload,
        ...
        if matches!(target, ProjectKnowledgeProposalTarget::NewProjectMemory)
            && source_run_id.is_none()
        {
            return Err(StoreError::Validation(
                "новой памяти нужен provenance запуска агента".into(),
            ));
        }
        ...
        if let Some(run_id) = source_run_id.as_deref()
            && let Some(existing) = self
                .list_project_knowledge_proposals(&project_id)?
                .into_iter()
                .find(|proposal| {
                    proposal.source_run_id.as_deref() == Some(run_id)
        ...
        let now = Utc::now();
        let mut proposal = ProjectKnowledgeProposal {
            id: Ulid::new().to_string(),
            project_id,
            target,
            base_version,
        ...
        proposal.version = project_knowledge_proposal_version(&proposal)?;
    ...
    pub fn apply_project_knowledge_proposal(
        ...
        ensure_version(&proposal.version, expected_version)?;
        ...
        let result = match (&proposal.target, &proposal.payload) {
            ...
            (
                ...
                .map(|_| ()),
            (
                ProjectKnowledgeProposalTarget::NewProjectMemory,
                ProjectKnowledgeProposalPayload::ProjectMemory { text, pinned },
            ) => {
                let source_run_id = proposal.source_run_id.as_deref().ok_or_else(|| {
                    StoreError::Validation("новой памяти нужен provenance запуска агента".into())
                })?;
                let source_task_id = self.get_agent_run(source_run_id)?.task_id;
                self.add_project_memory_idempotent(
                    project_id,
                    text,
                    Some(&source_task_id),
                    *pinned,
                    &proposal.base_version,
                    &format!("knowledge-proposal:{}", proposal.id),
                )
                .map(|_| ())
            }
    ...
    pub fn supersede_project_memory_idempotent(
        ...
        self.supersede_project_memory_with_reason_idempotent(
            project_id,
            memory_id,
            replacement_text,
            pinned,
    ...
    #[allow(clippy::too_many_arguments)]
    pub fn supersede_project_memory_with_reason_idempotent(
        &self,
        project_id: &str,
        memory_id: &str,
        replacement_text: &str,
        ...
        let reason = clean_optional(reason, "причина замены памяти", 500)?;
        let request_id = clean_required(request_id, "request_id", 200)?;
        ...
        project.memory[position].state_reason = reason;
    ...
    pub fn mark_project_memory_stale(
        &self,
        project_id: &str,
        memory_id: &str,
        reason: &str,
        ...
        validate_id(project_id)?;
        validate_id(memory_id)?;
        let reason = clean_required(reason, "причина устаревания памяти", 500)?;
        let _lock = self.lock_exclusive()?;
        let mut project = read_project(&self.project_path(project_id))
            .map_err(|error| map_missing(error, project_id))?;
        ensure_version(&project.version, expected_version)?;
        ...
        if entry.state != crate::ProjectMemoryState::Active {
            return Err(StoreError::Validation(
                "устаревшей можно пометить только актуальную запись".into(),
            ));
        }
        entry.state = crate::ProjectMemoryState::Stale;
        entry.state_reason = Some(reason);
        entry.pinned = false;
    ...
    pub fn update_agent_run(&self, id: &str, patch: AgentRunPatch) -> Result<AgentRun, StoreError> {
        ...
        if let Some(state) = patch.state {
            if !run.state.can_transition_to(&state) {
                return Err(StoreError::Validation(format!(
                    "недопустимый переход запуска агента: {:?} -> {:?}",
                    run.state, state
                )));
            }
            run.state = state;
    ...
    pub fn append_agent_run_receipt(
        &self,
        id: &str,
        receipt: crate::WorkPacketReceipt,
    ) -> Result<AgentRun, StoreError> {
        validate_id(id)?;
        validate_id(&receipt.turn_id)?;
        if receipt.run_id != id {
            return Err(StoreError::Validation(
                "receipt относится к другому запуску агента".into(),
            ));
        }
        let _lock = self.lock_exclusive()?;
        let mut document = self.read_agent_runs()?;
        ...
        if receipt.schema_version != 1
            || receipt.provider.capabilities.schema_version != 1
            || receipt.project_id != run.project_id
            || receipt.task_id.as_deref() != Some(run.task_id.as_str())
            || receipt.context_manifest.project_id != run.project_id
            || receipt.context_manifest.task_id.as_deref() != Some(run.task_id.as_str())
            || receipt.context_manifest.context_digest.trim().is_empty()
            || receipt.project_version.trim().is_empty()
            || receipt.task_version.as_deref().is_none_or(str::is_empty)
            || receipt.working_directory != run.working_directory
            || receipt.sandbox.trim().is_empty()
        {
            return Err(StoreError::Validation(
                "receipt не соответствует запуску или содержит неполный capability envelope".into(),
            ));
        }
        if let Some(existing) = run
            .turns
            .iter()
            .find(|item| item.turn_id == receipt.turn_id)
        {
            if existing == &receipt {
                return Ok(run.clone());
            }
        ...
        if run.turns.len() >= 32 {
            return Err(StoreError::Validation(
                "запуск достиг предела сохранённых turn receipts".into(),
            ));
        }
    ...
    pub fn accept_agent_run(
        ...
        ensure_version(&task.version, expected_task_version)?;
    ...
    pub fn preview_task_batch(
        ...
        if receipt_path.exists() {
            ...
            if receipt.rollback_in_progress {
                return Err(StoreError::Validation(
                    "rollback пакетного изменения ещё не завершён; переоткройте хранилище".into(),
                ));
            }
            if receipt.rolled_back {
                return Err(StoreError::Validation(
                    "request_id относится к уже откатанному пакетному изменению".into(),
                ));
            }
    ...
    pub fn apply_task_batch(
        ...
        self.apply_task_batch_with_hook(operations, expected_versions, request_id, |_| {})
    }

    fn apply_task_batch_with_hook<F>(
        &self,
        operations: Vec<TaskBatchOperation>,
        expected_versions: Vec<crate::ExpectedTaskVersion>,
        request_id: &str,
    ...
    where
        F: FnMut(TaskBatchWriteBoundary),
    {
        if operations.is_empty() || operations.len() > MAX_TASK_BATCH_OPERATIONS {
            return Err(StoreError::Validation(format!(
                "пакет должен содержать от 1 до {MAX_TASK_BATCH_OPERATIONS} действий"
        ...
        if receipt_path.exists() {
            ...
            if receipt.rolled_back {
                return Err(StoreError::Validation(
                    "request_id относится к уже откатанному пакетному изменению; используйте новый request_id".into(),
                ));
            }
            if receipt.applied {
            ...
            if receipt.format_version == 1 {
                let (receipt, tasks) = self.finish_task_batch_receipt(receipt, &receipt_path)?;
                return Ok(TaskBatchOutcome {
                    request_id,
                    repeated: true,
                    operations: receipt.operations,
            ...
            self.restore_task_batch_artifacts(&receipt.artifacts)?;
            fs::remove_file(&receipt_path)?;
        }

        let (operation_results, tasks) =
            self.prepare_task_batch_locked(&operations, &expected_versions, &request_id)?;
        let artifacts = self.capture_task_batch_artifacts(&tasks)?;
        ...
        after_write(TaskBatchWriteBoundary::JournalPrepared);

        if let Err(error) = self.apply_task_batch_tasks_with_hook(&receipt.tasks, &mut after_write)
        {
            let rollback = self.restore_task_batch_artifacts(&receipt.artifacts);
            if rollback.is_ok() {
                let _ = fs::remove_file(&receipt_path);
                return Err(error);
            }
            return Err(StoreError::Io(std::io::Error::other(format!(
                "пакет не удалось записать и полностью откатить; журнал восстановления сохранён: {error}"
            ))));
        }
        ...
        let tasks = receipt
            ...
            .collect::<Result<Vec<_>, _>>()?;
        for task in &tasks {
            let Some(artifact) = receipt
                .artifacts
                .iter()
                .find(|artifact| artifact.task_id == task.id)
            else {
                return Err(StoreError::Validation(
                    "журнал пакетного изменения не содержит снимок изменённой задачи".into(),
                ));
            };
            if task.version != artifact.target_version {
                return Err(StoreError::Conflict);
            }
        }
        ...
        if let Err(error) = self.write_task_batch_receipt(&receipt_path, &receipt) {
            let rollback = self.restore_task_batch_artifacts(&receipt.artifacts);
            if rollback.is_ok() {
                let _ = fs::remove_file(&receipt_path);
                return Err(error);
            ...
            return Err(StoreError::Io(std::io::Error::other(format!(
                "результат пакета записан, но commit-журнал не сохранён и автоматический откат не завершён: {error}"
            ))));
        }
        after_write(TaskBatchWriteBoundary::JournalCommitted);
        Ok(TaskBatchOutcome {
            request_id,
            repeated: false,
            operations: receipt.operations,
    ...
    pub fn rollback_task_batch(
        &self,
        request_id: &str,
    ) -> Result<TaskBatchRollbackOutcome, StoreError> {
        let request_id = clean_required(request_id, "request_id", 200)?;
        let receipt_path = self.task_batch_receipt_path(&request_id);
        let _lock = self.lock_exclusive()?;
        if !receipt_path.exists() {
        ...
        let mut receipt = self.read_task_batch_receipt(&receipt_path)?;
        if receipt.request_id != request_id {
            return Err(StoreError::Validation(
                "журнал пакетного изменения не соответствует request_id".into(),
            ));
        }
        if receipt.format_version < 2 {
            return Err(StoreError::Validation(
                "legacy-журнал не содержит versioned snapshots для безопасного rollback".into(),
            ));
        }
        let restored_task_ids = receipt
            .artifacts
            .iter()
            .filter(|artifact| artifact.original_markdown.is_some())
            .map(|artifact| artifact.task_id.clone())
            .collect();
        let removed_task_ids = receipt
            .artifacts
            .iter()
            .filter(|artifact| artifact.original_markdown.is_none())
            .map(|artifact| artifact.task_id.clone())
            .collect();
        if receipt.rolled_back {
            return Ok(TaskBatchRollbackOutcome {
                request_id,
                repeated: true,
                restored_task_ids,
                removed_task_ids,
        ...
        let actions = self.plan_task_batch_restore(&receipt.artifacts)?;
        receipt.rollback_in_progress = true;
        self.write_task_batch_receipt(&receipt_path, &receipt)?;
        self.execute_task_batch_restore_actions(actions)?;
        receipt.applied = false;
        receipt.rolled_back = true;
        receipt.rollback_in_progress = false;
        receipt.tasks.clear();
        self.write_task_batch_receipt(&receipt_path, &receipt)?;
        Ok(TaskBatchRollbackOutcome {
            request_id,
            repeated: false,
            restored_task_ids,
            removed_task_ids,
    ...
    pub fn create_backup(&self, destination: &Path) -> Result<(), StoreError> {
        ...
        let result = (|| {
            ...
            rename_with_transient_retry(&temporary, destination)?;
            Ok(())
        })();
        if result.is_err() {
    ...
    pub fn export_project(
        ...
        project_id: &str,
        destination: &Path,
    ) -> Result<ProjectExportManifest, StoreError> {
        validate_id(project_id)?;
        let parent = destination
            .parent()
            .ok_or_else(|| StoreError::Backup("некорректный путь экспорта".into()))?;
        fs::create_dir_all(parent)?;
        let destination_parent = fs::canonicalize(parent)?;
        let root = fs::canonicalize(&self.root)?;
        if destination_parent.starts_with(root) {
            return Err(StoreError::Backup(
                "экспорт проекта нельзя сохранить внутри данных flood.md".into(),
            ));
        }
        let _lock = self.lock_shared()?;
        let project = read_project(&self.project_path(project_id))
            .map_err(|error| map_missing(error, project_id))?;
        let project_directory = self.projects_dir().join(project_id);
        let mut paths = Vec::new();
        collect_backup_files(&project_directory, &mut paths)?;
        paths.sort();
        ...
        for path in paths {
            let relative = path
                ...
                .map_err(|_| StoreError::Backup("некорректный путь проекта".into()))?;
            let relative = relative.to_string_lossy().replace('\\', "/");
            let bytes = fs::read(&path)?;
            files.push(ProjectExportFile {
                path: relative.clone(),
                bytes: bytes.len() as u64,
                sha256: digest(&bytes),
            });
            content.push((relative, bytes));
        }
        ...
        let temporary = parent.join(format!(".flood-project-export-{}.tmp", Ulid::new()));
        let result = (|| {
            let file = File::create(&temporary)?;
            let mut archive = ZipWriter::new(file);
            let options = SimpleFileOptions::default()
                ...
                .unix_permissions(0o600);
            archive
                .start_file("flood-project-export.json", options)
                .map_err(|error| StoreError::Backup(error.to_string()))?;
            archive.write_all(&serde_json::to_vec_pretty(&manifest)?)?;
            for (relative, bytes) in &content {
                archive
                    .start_file(format!("project/{relative}"), options)
                    .map_err(|error| StoreError::Backup(error.to_string()))?;
    ...
    pub fn import_project(&self, source: &Path) -> Result<ProjectImportReport, StoreError> {
        if !source.is_file() {
            return Err(StoreError::Backup("файл экспорта проекта не найден".into()));
        }
        let import_root = self.root.join(format!(".project-import-{}", Ulid::new()));
        let result = (|| {
            fs::create_dir_all(&import_root)?;
            let mut archive = ZipArchive::new(File::open(source)?)
                .map_err(|error| StoreError::Backup(error.to_string()))?;
            if archive.len() > 20_001 {
                return Err(StoreError::Backup("в экспорте слишком много файлов".into()));
            }
            let mut manifest = None;
            let mut extracted = HashMap::<String, PathBuf>::new();
            ...
            for index in 0..archive.len() {
                ...
                let relative = entry
                    .enclosed_name()
                    .ok_or_else(|| StoreError::Backup("экспорт содержит небезопасный путь".into()))?
                    .to_path_buf();
                if entry
                    .unix_mode()
                ...
                {
                    return Err(StoreError::Backup(
                        "символические ссылки в экспорте не поддерживаются".into(),
                    ));
                }
                total_size = total_size.saturating_add(entry.size());
                if total_size > 1024 * 1024 * 1024 {
                    return Err(StoreError::Backup("экспорт превышает 1 ГБ".into()));
                }
                if relative == Path::new("flood-project-export.json") {
                    let mut bytes = Vec::new();
                    entry
                        .by_ref()
                        .take(MAX_MARKDOWN_FILE_BYTES + 1)
                        .read_to_end(&mut bytes)?;
                    if bytes.len() as u64 > MAX_MARKDOWN_FILE_BYTES {
                        return Err(StoreError::Backup("manifest экспорта слишком велик".into()));
                    }
                    manifest = Some(serde_json::from_slice::<ProjectExportManifest>(&bytes)?);
                    continue;
                }
                let item = relative.strip_prefix("project").map_err(|_| {
                    StoreError::Backup("архив не является экспортом проекта flood.md".into())
                })?;
                if item.as_os_str().is_empty() || entry.is_dir() {
                    continue;
                }
                let key = item.to_string_lossy().replace('\\', "/");
                let output = import_root.join("project").join(item);
                if extracted.contains_key(&key) {
                    return Err(StoreError::Backup(format!(
                        "экспорт содержит повторяющийся путь {key}"
                    )));
                }
                let virtual_project_path = Path::new("projects").join("x").join(item);
                let limit = restore_entry_size_limit(&virtual_project_path);
                if entry.size() > limit {
                    return Err(StoreError::Backup(format!(
                        "файл {key} превышает безопасный предел"
                    )));
                }
                ...
                let copied = std::io::copy(&mut entry.by_ref().take(limit + 1), &mut target)?;
                if copied > limit {
                    return Err(StoreError::Backup(format!(
                        "файл {key} превысил безопасный предел при распаковке"
                    )));
                }
                extracted_size = extracted_size.saturating_add(copied);
                if extracted_size > 1024 * 1024 * 1024 {
                    return Err(StoreError::Backup(
                        "распакованный проект превышает 1 ГБ".into(),
                    ));
                }
                target.sync_all()?;
                extracted.insert(key, output);
            }
            let manifest = manifest
                .ok_or_else(|| StoreError::Backup("в экспорте отсутствует manifest".into()))?;
            if manifest.schema_version != 1 {
                return Err(StoreError::Backup(format!(
                    "неподдерживаемая версия экспорта {}",
            ...
            let manifest_paths = manifest
                ...
                .collect::<HashSet<_>>();
            let extracted_paths = extracted.keys().map(String::as_str).collect::<HashSet<_>>();
            if manifest_paths.len() != manifest.files.len() || manifest_paths != extracted_paths {
                return Err(StoreError::Backup(
                    "набор файлов не совпадает с manifest".into(),
                ));
            }
            for item in &manifest.files {
                let path = extracted
                    .get(&item.path)
                    .ok_or_else(|| StoreError::Backup(format!("отсутствует файл {}", item.path)))?;
                let bytes = fs::read(path)?;
                if bytes.len() as u64 != item.bytes || digest(&bytes) != item.sha256 {
                    return Err(StoreError::Backup(format!(
                        "не прошла проверка целостности {}",
                        item.path
                    )));
                }
            }
            let imported_project = read_project(&import_root.join("project/project.md"))?;
            if imported_project.id != manifest.project_id
                || imported_project.version != manifest.project_version
            {
                return Err(StoreError::Backup(
                    "identity/version проекта не совпадает с manifest".into(),
                ));
            }
            for (relative, path) in &extracted {
                let expected_kind =
                    if relative.starts_with("documents/") && relative.ends_with(".md") {
                        Some(ProjectWorkspaceItemKind::Document)
                    } else if relative.starts_with("rules/") && relative.ends_with(".md") {
                        Some(ProjectWorkspaceItemKind::Rule)
                    } else if relative.starts_with("skills/") && relative.ends_with(".md") {
                        Some(ProjectWorkspaceItemKind::Skill)
                    } else {
                        None
                    };
                if let Some(expected_kind) = expected_kind {
                    let item = read_project_workspace_item(path)?;
                    if item.project_id != manifest.project_id || item.kind != expected_kind {
                        return Err(StoreError::Backup(format!(
                            "материал {relative} принадлежит другому проекту или разделу"
                        )));
                    }
                }
                if relative.starts_with("proposals/") && relative.ends_with(".json") {
                    let proposal = self.read_project_knowledge_proposal(path)?;
                    if proposal.project_id != manifest.project_id {
                        return Err(StoreError::Backup(format!(
                            "proposal {relative} принадлежит другому проекту"
                        )));
                    }
                }
            }
            let mut conflicts = Vec::new();
            let destination = self.projects_dir().join(&manifest.project_id);
            if destination.exists() {
                conflicts.push(ProjectImportConflict {
                    kind: "project_id".into(),
                    id: manifest.project_id.clone(),
                    reason: "проект с таким stable id уже существует; данные не перезаписаны"
                        .into(),
                });
            }
            if let Some(tasks) = extracted
                .keys()
                .any(|path| path.starts_with("tasks/") && path.ends_with(".md"))
                .then_some(import_root.join("project/tasks"))
                .filter(|path| path.is_dir())
            {
                for entry in fs::read_dir(tasks)? {
                    ...
                    if task.project_id != manifest.project_id {
                        return Err(StoreError::Backup(format!(
                            "задача {} ссылается на другой проект",
                            task.id
                        )));
                    }
                    if self.find_task(&task.id).is_ok() {
                        conflicts.push(ProjectImportConflict {
                            kind: "task_id".into(),
                            id: task.id,
                            reason: "задача с таким stable id уже существует".into(),
                        });
                    }
                }
            }
            ...
            let _lock = self.lock_exclusive()?;
            rename_with_transient_retry(&import_root.join("project"), &destination)?;
            read_project(&destination.join("project.md"))?;
    ...
    pub fn restore_backup(&self, source: &Path) -> Result<(), StoreError> {
        ...
        let result = (|| {
            ...
            rename_with_transient_retry(&current_projects, &previous_projects)?;
            if let Err(error) =
                rename_with_transient_retry(&restore_root.join("projects"), &current_projects)
            {
                let _ = rename_with_transient_retry(&previous_projects, &current_projects);
                return Err(error.into());
            }
            ...
            if current_integrations.exists()
                && let Err(error) =
                    rename_with_transient_retry(&current_integrations, &previous_integrations)
            {
            ...
            if restored_integrations.exists()
                && let Err(error) =
                    rename_with_transient_retry(&restored_integrations, &current_integrations)
            {
                let _ = fs::remove_dir_all(&current_projects);
                let _ = rename_with_transient_retry(&previous_projects, &current_projects);
                if previous_integrations.exists() {
                    let _ =
                        rename_with_transient_retry(&previous_integrations, &current_integrations);
                }
                return Err(error.into());
            }
        ...
        if result.is_err() && previous_projects.exists() && !self.projects_dir().exists() {
            let _ = rename_with_transient_retry(&previous_projects, &self.projects_dir());
        }
        if result.is_err()
            && previous_integrations.exists()
            && !self.root.join("integrations").exists()
        {
            let _ = rename_with_transient_retry(
    ...
    fn read_project_knowledge_proposal(
        ...
        let version = project_knowledge_proposal_version(&proposal)?;
        if proposal.version.is_empty() {
            proposal.version = version;
        } else if proposal.version != version {
            return Err(StoreError::InvalidFile {
                path: path.to_string_lossy().into_owned(),
                message: "версия proposal не совпадает с содержимым".into(),
            });
        }
        Ok(proposal)
    }
    ...
    fn capture_task_batch_artifacts(
        &self,
        tasks: &[Task],
    ) -> Result<Vec<TaskBatchArtifactSnapshot>, StoreError> {
        tasks
            .iter()
            .map(|task| {
                let path = self.task_path(&task.project_id, &task.id);
                let original_markdown = path
                    .exists()
                    .then(|| read_limited_utf8(&path, MAX_MARKDOWN_FILE_BYTES))
                    .transpose()?;
                let original_version = original_markdown
                    .as_ref()
                    .map(|content| digest(content.as_bytes()));
                if let Some(version) = original_version.as_deref() {
                    if task.version.is_empty() || task.version != version {
                        return Err(StoreError::Conflict);
                    }
                } else if !task.version.is_empty() {
                    return Err(StoreError::Conflict);
                }
                let target_markdown = encode_task(task)?;
                Ok(TaskBatchArtifactSnapshot {
                    project_id: task.project_id.clone(),
                    task_id: task.id.clone(),
                    original_markdown,
                    original_version,
                    target_version: digest(target_markdown.as_bytes()),
                })
            })
            .collect()
    }

    fn ensure_created_batch_tasks_unreferenced(
        &self,
        artifacts: &[TaskBatchArtifactSnapshot],
    ) -> Result<(), StoreError> {
        let receipt_task_ids = artifacts
            .iter()
            .map(|artifact| artifact.task_id.as_str())
            .collect::<HashSet<_>>();
        let created_task_ids = artifacts
            .iter()
            .filter(|artifact| artifact.original_markdown.is_none())
            .map(|artifact| artifact.task_id.as_str())
            .collect::<HashSet<_>>();
        ...
        for project in fs::read_dir(self.projects_dir())? {
            ...
            for entry in fs::read_dir(tasks_dir)? {
                ...
                let task = read_task(&path)?;
                if receipt_task_ids.contains(task.id.as_str()) {
                    continue;
                }
                if task
                    .relations
                    .iter()
                    .any(|relation| created_task_ids.contains(relation.task_id.as_str()))
                {
                    return Err(StoreError::Conflict);
                }
            }
    ...
    fn plan_task_batch_restore(
        &self,
        artifacts: &[TaskBatchArtifactSnapshot],
    ) -> Result<Vec<TaskBatchRestoreAction>, StoreError> {
        self.ensure_created_batch_tasks_unreferenced(artifacts)?;
        let mut actions = Vec::new();
        for artifact in artifacts.iter().rev() {
            validate_id(&artifact.project_id)?;
            validate_id(&artifact.task_id)?;
            if artifact.target_version.is_empty() {
                return Err(StoreError::Validation(
                    "журнал пакетного изменения не содержит target version".into(),
                ));
            }
            let path = self.task_path(&artifact.project_id, &artifact.task_id);
            let current_markdown = path
                .exists()
                .then(|| read_limited_utf8(&path, MAX_MARKDOWN_FILE_BYTES))
                .transpose()?;
            let current_version = current_markdown
                .as_ref()
                .map(|content| digest(content.as_bytes()));

            match (
                artifact.original_markdown.as_deref(),
                artifact.original_version.as_deref(),
            ) {
                (Some(original), Some(original_version)) => {
                    if digest(original.as_bytes()) != original_version {
                        return Err(StoreError::Validation(
                            "исходный снимок в журнале пакетного изменения повреждён".into(),
                        ));
                    }
                    match current_version.as_deref() {
                        Some(version) if version == original_version => {}
                        Some(version) if version == artifact.target_version => {
                            actions
                                .push(TaskBatchRestoreAction::Restore(path, original.to_owned()));
                        }
                        _ => return Err(StoreError::Conflict),
                    }
                }
                (None, None) => match current_version.as_deref() {
                    None => {}
                    Some(version) if version == artifact.target_version => {
                        actions.push(TaskBatchRestoreAction::Remove(path));
                    }
                    _ => return Err(StoreError::Conflict),
                },
                _ => {
                    return Err(StoreError::Validation(
                        "журнал пакетного изменения содержит неполный исходный снимок".into(),
                    ));
                }
            }
        }
    ...
    fn execute_task_batch_restore_actions(
        &self,
        actions: Vec<TaskBatchRestoreAction>,
    ) -> Result<(), StoreError> {
        for action in actions {
            match action {
                TaskBatchRestoreAction::Restore(path, original) => {
                    atomic_write_bytes(&path, original.as_bytes())?;
                }
                TaskBatchRestoreAction::Remove(path) => {
                    if path.exists() {
                        fs::remove_file(path)?;
                    }
                }
    ...
    fn restore_task_batch_artifacts(
        &self,
        artifacts: &[TaskBatchArtifactSnapshot],
    ) -> Result<(), StoreError> {
        let actions = self.plan_task_batch_restore(artifacts)?;
        self.execute_task_batch_restore_actions(actions)
    }
    ...
    fn apply_task_batch_tasks_with_hook<F>(
        &self,
        tasks: &[Task],
        after_write: &mut F,
    ) -> Result<(), StoreError>
        ...
        F: FnMut(TaskBatchWriteBoundary),
    {
        for task in tasks {
            self.write_task(task)?;
            after_write(TaskBatchWriteBoundary::TaskWritten);
        }
        Ok(())
    }
    ...
    fn read_task_batch_receipt(&self, path: &Path) -> Result<TaskBatchReceipt, StoreError> {
        ...
        if receipt.rolled_back && (receipt.applied || receipt.rollback_in_progress) {
            return Err(StoreError::Validation(
                "журнал пакетного изменения содержит несовместимые состояния".into(),
            ));
        }
        if receipt.format_version == 2
            && !receipt.rolled_back
            && !receipt.applied
            && receipt.tasks.len() != receipt.artifacts.len()
        {
            return Err(StoreError::Validation(
                "журнал пакетного изменения содержит неполный recovery snapshot".into(),
            ));
        }
        Ok(receipt)
    }
    ...
    fn write_task_batch_receipt(
        ...
        if bytes.len() as u64 > MAX_TASK_BATCH_RECEIPT_BYTES {
            return Err(StoreError::Validation(
                "журнал пакетного изменения превышает безопасный размер 64 МБ".into(),
            ));
        }
        atomic_write_bytes(path, &bytes)
    }
    ...
    fn recover_task_batches_locked(&self) -> Result<(), StoreError> {
        ...
        for entry in fs::read_dir(directory)? {
            ...
            let mut receipt = self.read_task_batch_receipt(&path)?;
            if receipt.rolled_back {
                continue;
            }
            if receipt.rollback_in_progress {
...
fn atomic_write(path: &Path, content: &str) -> Result<(), StoreError> {
    ...
    atomic_write_with(path, content.as_bytes(), |file, bytes| {
        file.write_all(bytes)
    })
}

fn atomic_write_with<F>(path: &Path, content: &[u8], write: F) -> Result<(), StoreError>
where
    F: FnOnce(&mut AtomicWriteFile, &[u8]) -> std::io::Result<()>,
{
    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent)?;
    }
...
fn atomic_write_bytes(path: &Path, content: &[u8]) -> Result<(), StoreError> {
    atomic_write_with(path, content, |file, bytes| file.write_all(bytes))
}
...
fn project_knowledge_proposal_version(
    proposal: &ProjectKnowledgeProposal,
) -> Result<String, StoreError> {
    let reviewable = (
        &proposal.id,
        ...
        &proposal.decision_provenance,
        &proposal.created_at,
        &proposal.updated_at,
    );
    Ok(digest(&serde_json::to_vec(&reviewable)?))
}
...
fn project_knowledge_payload_matches_current(
    ...
    match (&proposal.target, &proposal.payload) {
        ...
        (
            ...
            let project = store.get_project(&proposal.project_id)?;
            Ok(project.memory.iter().any(|entry| {
                entry.state.is_active() && entry.text == *text && entry.pinned == *pinned
            }))
        }
        _ => Ok(false),
    }
...
mod tests {
    use super::*;
    use crate::{
        AppliedGuidance, ContextBuilder, GuidanceKind, GuidanceReference, ProviderCapabilities,
        ProviderDescriptor, WorkAction, WorkPacketReceipt, WorkPurpose,
    };
    ...
    #[test]
    fn storage_full_before_commit_preserves_previous_canonical_file() {
        let root = env::temp_dir().join(format!("flood-disk-full-{}", Ulid::new()));
        fs::create_dir_all(&root).unwrap();
        let path = root.join("task.md");
        fs::write(&path, "canonical-before").unwrap();

        let error = atomic_write_with(&path, b"replacement-after", |file, bytes| {
        ...
        assert!(
            matches!(error, StoreError::Io(ref inner) if inner.kind() == std::io::ErrorKind::StorageFull)
        );
        assert_eq!(fs::read_to_string(&path).unwrap(), "canonical-before");
    ...
    #[test]
    fn agent_turn_receipt_is_exact_immutable_and_persistent() {
        let store = temp_store();
        let project = store.create_project("Agent receipt").unwrap();
        let task = store
            .create_task(CreateTask {
                project_id: project.id,
                description: "Проверить provenance".into(),
                urgency: Urgency::Normal,
                source: None,
            })
            .unwrap();
        ...
        let saved = store
            .append_agent_run_receipt(&run.id, receipt.clone())
            .unwrap();
        assert_eq!(saved.turns, vec![receipt.clone()]);
        assert_eq!(
            store
                .append_agent_run_receipt(&run.id, receipt.clone())
                .unwrap()
        ...
        changed.permissions.push("external:write".into());
        assert!(matches!(
            store.append_agent_run_receipt(&run.id, changed),
            Err(StoreError::Conflict)
        ));

        let serialized = serde_json::to_string(&receipt).unwrap();
        assert!(!serialized.contains("Проверить provenance"));
        store
            .update_agent_run(
                &run.id,
                AgentRunPatch {
        ...
        store
            ...
            .unwrap();
        let current_project = store.get_project(&task.project_id).unwrap();
        let proposal = store
            .create_project_knowledge_proposal_for_run(
                NewProjectKnowledgeProposal {
                    project_id: current_project.id.clone(),
                    target: ProjectKnowledgeProposalTarget::NewProjectMemory,
                    base_version: current_project.version,
                    payload: ProjectKnowledgeProposalPayload::ProjectMemory {
                        text: "Результаты проверять на реальных данных".into(),
                        pinned: false,
                    },
                    summary: "Сохранить устойчивый вывод".into(),
                    reason: "Подтверждено выполнением задачи".into(),
                    evidence: vec!["verification:contract".into()],
                    provenance: None,
                },
                &run.id,
            )
            .unwrap();
        store
            .apply_project_knowledge_proposal(&current_project.id, &proposal.id, &proposal.version)
            .unwrap();
        assert!(
            store
                .get_project(&current_project.id)
                .unwrap()
                .memory
                .iter()
                .any(|entry| {
                    entry.text == "Результаты проверять на реальных данных"
                        && entry.source_task_id.as_deref() == Some(task.id.as_str())
                })
        );
        assert_eq!(
    ...
    #[test]
    fn accepting_agent_result_keeps_task_open_and_is_retry_safe() {
        let store = temp_store();
        ...
        assert_eq!(completed.status, TaskStatus::Open);
    ...
    #[test]
    fn legacy_pending_task_batch_is_recovered_by_roll_forward() {
        let store = temp_store();
        let project = store.create_project("Восстановление legacy").unwrap();
        let outcome = store
            .apply_task_batch(
    ...
    fn knowledge_proposal_survives_restart_rejects_stale_base_and_retries_apply() {
        ...
        let item = store
            ...
            .value;
        let stale = store
            .create_project_knowledge_proposal(NewProjectKnowledgeProposal {
                project_id: project.id.clone(),
                target: ProjectKnowledgeProposalTarget::WorkspaceItem {
                    item_id: item.id.clone(),
                    item_kind: item.kind,
                },
                base_version: item.version.clone(),
                ...
                summary: "Обновить правило".into(),
                reason: "Новая проверенная договорённость".into(),
                evidence: vec!["issue-29".into()],
                provenance: None,
            })
            .unwrap();
        ...
        let concurrent = reopened
            ...
            .unwrap();
        assert!(matches!(
            reopened.apply_project_knowledge_proposal(&project.id, &stale.id, &stale.version),
            Err(StoreError::Conflict)
        ));
        ...
        let proposal = reopened
            .create_project_knowledge_proposal(NewProjectKnowledgeProposal {
                ...
                base_version: concurrent.version.clone(),
                payload: ProjectKnowledgeProposalPayload::WorkspaceItem {
                ...
                summary: "Применить итог".into(),
                reason: "Ревью завершено".into(),
                evidence: Vec::new(),
                provenance: None,
            })
            .unwrap();
        assert!(matches!(
            reopened.apply_project_knowledge_proposal(
                &project.id,
                &proposal.id,
                "reviewed-an-older-payload"
            ),
            Err(StoreError::Conflict)
        ));
        let applied = reopened
            .apply_project_knowledge_proposal(&project.id, &proposal.id, &proposal.version)
            .unwrap();
    ...
    #[test]
    fn task_batch_rollback_restores_exact_versions_and_is_idempotent() {
        let store = temp_store();
        let project = store.create_project("Rollback").unwrap();
        let existing = store
            .create_task(CreateTask {
                project_id: project.id.clone(),
                description: "# Исходная".into(),
                urgency: Urgency::Normal,
                source: None,
            })
            .unwrap();
        let request_id = "batch-rollback";
        let outcome = store
            .apply_task_batch(
                vec![
                    TaskBatchOperation::Update {
                        ...
                        patch: TaskPatch {
                            description: Some("# Изменённая".into()),
                            ..TaskPatch::default()
                        },
                    },
                    TaskBatchOperation::Create {
                        operation_id: "created".into(),
                        project_id: project.id.clone(),
                        description: "# Временная".into(),
                        urgency: Urgency::Important,
                        source: None,
                    },
                ],
                vec![crate::ExpectedTaskVersion {
                    task_id: existing.id.clone(),
                    version: existing.version.clone(),
                }],
                request_id,
        ...
        let created_id = outcome
            .operations
            .iter()
            .find(|operation| operation.operation_id == "created")
            .unwrap()
            .task_id
            .clone();

        let rollback = store.rollback_task_batch(request_id).unwrap();
        assert!(!rollback.repeated);
        assert_eq!(rollback.restored_task_ids, vec![existing.id.clone()]);
        assert_eq!(rollback.removed_task_ids, vec![created_id.clone()]);
        let restored = store.get_task(&existing.id).unwrap();
        assert_eq!(restored.description, "# Исходная");
        assert_eq!(restored.version, existing.version);
        assert!(matches!(
            store.get_task(&created_id),
            Err(StoreError::NotFound(_))
        ));

        let repeated = store.rollback_task_batch(request_id).unwrap();
        assert!(repeated.repeated);
        assert_eq!(
            repeated,
            TaskBatchRollbackOutcome {
                request_id: request_id.into(),
                repeated: true,
                restored_task_ids: vec![existing.id],
                removed_task_ids: vec![created_id],
            }
        );
    }

    #[test]
    fn task_batch_rollback_preflights_all_versions_before_restoring_any_artifact() {
        let store = temp_store();
        let project = store.create_project("Rollback conflict").unwrap();
        let first = store
            .create_task(CreateTask {
                project_id: project.id.clone(),
                description: "# Первая исходная".into(),
                urgency: Urgency::Normal,
                source: None,
            })
            .unwrap();
        let second = store
            .create_task(CreateTask {
                project_id: project.id,
                description: "# Вторая исходная".into(),
                urgency: Urgency::Normal,
                source: None,
            })
            .unwrap();
        let request_id = "batch-rollback-conflict";
        let applied = store
            .apply_task_batch(
                vec![
                    TaskBatchOperation::Update {
                        ...
                        patch: TaskPatch {
                            description: Some("# Первая после пакета".into()),
                            ..TaskPatch::default()
                        },
                    },
                    TaskBatchOperation::Update {
                        operation_id: "update-second".into(),
                        task: TaskBatchReference {
                            task_id: Some(second.id.clone()),
                            operation_id: None,
                        },
                        patch: TaskPatch {
                            description: Some("# Вторая после пакета".into()),
                            ..TaskPatch::default()
                        },
                    },
                ],
                vec![
                    crate::ExpectedTaskVersion {
        ...
        store
            .update_task(
                &second.id,
                TaskPatch {
                    description: Some("# Более новая ручная правка".into()),
                    ..TaskPatch::default()
                },
                &second_applied.version,
            )
            .unwrap();

        assert!(matches!(
            store.rollback_task_batch(request_id),
            Err(StoreError::Conflict)
        ));
        assert_eq!(
            store.get_task(&first.id).unwrap().description,
            "# Первая после пакета"
        );
        assert_eq!(
            store.get_task(&second.id).unwrap().description,
            "# Более новая ручная правка"
        );
    }
    ...
    fn rejected_knowledge_proposal_is_persistent_and_idempotent() {
        ...
        let proposal = store
            .create_project_knowledge_proposal(NewProjectKnowledgeProposal {
                ...
                summary: "Изменить документ".into(),
    ...
    #[test]
    fn task_batch_rollback_rejects_created_task_with_new_external_dependent() {
        let store = temp_store();
        let project = store.create_project("Rollback dependency").unwrap();
        let dependent = store
            .create_task(CreateTask {
                project_id: project.id.clone(),
                description: "# Поздняя зависимость".into(),
                urgency: Urgency::Normal,
                source: None,
            })
            .unwrap();
        let request_id = "batch-rollback-dependent";
        let applied = store
            .apply_task_batch(
                vec![TaskBatchOperation::Create {
                    operation_id: "created".into(),
                    project_id: project.id,
                    description: "# Созданная пакетом".into(),
                    urgency: Urgency::Normal,
                    source: None,
                }],
                Vec::new(),
            ...
            .unwrap();
        let created = applied.tasks[0].clone();
        let dependent = store
            .link_tasks(
                &dependent.id,
                &created.id,
        ...
        assert_eq!(
            store.get_task(&created.id).unwrap().description,
            "# Созданная пакетом"
        );
        assert!(
            dependent
                .relations
                .iter()
                .any(|relation| relation.task_id == created.id)
        );
    }

    #[test]
    fn interrupted_committed_rollback_is_finalized_on_restart() {
        let store = temp_store();
        let root = store.root().to_path_buf();
        let project = store.create_project("Rollback restart").unwrap();
        let existing = store
            .create_task(CreateTask {
                project_id: project.id,
                description: "# До изменения".into(),
                urgency: Urgency::Normal,
                source: None,
            })
            .unwrap();
        let request_id = "batch-rollback-restart";
        store
            .apply_task_batch(
                vec![TaskBatchOperation::Update {
                    operation_id: "update".into(),
                    task: TaskBatchReference {
                        task_id: Some(existing.id.clone()),
                        operation_id: None,
                    ...
                    patch: TaskPatch {
                        description: Some("# После изменения".into()),
                        ..TaskPatch::default()
                    },
                }],
                vec![crate::ExpectedTaskVersion {
        ...
        let mut receipt = store.read_task_batch_receipt(&receipt_path).unwrap();
        let actions = store.plan_task_batch_restore(&receipt.artifacts).unwrap();
        receipt.rollback_in_progress = true;
        store
            .write_task_batch_receipt(&receipt_path, &receipt)
            .unwrap();
        store.execute_task_batch_restore_actions(actions).unwrap();
        assert_eq!(
            store.get_task(&existing.id).unwrap().description,
            "# До изменения"
        );

        let reopened = Store::new(&root).unwrap();
        let receipt = reopened.read_task_batch_receipt(&receipt_path).unwrap();
        assert!(receipt.rolled_back);
        assert!(!receipt.applied);
        assert!(!receipt.rollback_in_progress);
        assert_eq!(
            reopened.get_task(&existing.id).unwrap().description,
            "# До изменения"
        );
        assert!(reopened.rollback_task_batch(request_id).unwrap().repeated);
    }

    #[test]
    fn task_batch_fault_injection_recovers_every_write_boundary_after_restart() {
        for fail_at in 0..4 {
            let store = temp_store();
            let root = store.root().to_path_buf();
            let project = store.create_project("Fault recovery").unwrap();
            let existing = store
                .create_task(CreateTask {
                    project_id: project.id.clone(),
                    description: "# Исходная".into(),
                ...
                .unwrap();
            let request_id = format!("batch-fault-{fail_at}");
            let operations = vec![
                TaskBatchOperation::Update {
                    operation_id: "update".into(),
                    task: TaskBatchReference {
                    ...
                    patch: TaskPatch {
                        description: Some("# После пакета".into()),
                        ..TaskPatch::default()
                    },
                },
                TaskBatchOperation::Create {
            ...
            let created_id = deterministic_id(&format!("task-batch:{request_id}"), "created");
            let mut boundary = 0usize;

            let crashed = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                let _ = store.apply_task_batch_with_hook(
                    operations.clone(),
                    versions.clone(),
                    &request_id,
                    |_| {
                        if boundary == fail_at {
                            panic!("fault injection at write boundary {fail_at}");
                        }
                        boundary += 1;
                    },
                );
            ...
            let reopened = Store::new(&root).unwrap();
            if fail_at < 3 {
                let restored = reopened.get_task(&existing.id).unwrap();
                assert_eq!(restored.description, "# Исходная");
                assert_eq!(restored.version, existing.version);
                assert!(matches!(
                    reopened.get_task(&created_id),
                    Err(StoreError::NotFound(_))
                ));
                assert!(!reopened.task_batch_receipt_path(&request_id).exists());

                let retry = reopened
                    .apply_task_batch(operations, versions, &request_id)
                    .unwrap();
                ...
                assert_eq!(
                    reopened.get_task(&created_id).unwrap().description,
                    "# Созданная"
                );
            } else {
                assert_eq!(
