                ...
                Some(cursor) => cursor
                    ...
                    .ok_or_else(|| {
                        ConnectorOperationError::InvalidRequest(
                            "invalid Telegram source cursor".into(),
                        )
                    })?,
            };
    ...
    fn read_timeline(&self, request: TimelineRequest) -> ConnectorFuture<'_, ContextBatch> {
        ...
        Box::pin(async move {
            ...
            let chat_id = request
                ...
                .ok_or_else(|| {
                    ConnectorOperationError::InvalidRequest("invalid Telegram source id".into())
                })?;
            let after_message_id = match request.cursor.as_deref() {
                None => None,
                Some(cursor) => Some(
                    cursor
                        ...
                        .ok_or_else(|| {
                            ConnectorOperationError::InvalidRequest(
                                "invalid Telegram timeline cursor".into(),
                            )
                        })?,
                ),
            };
...
mod tests {
    ...
    #[test]
    fn local_ingestion_has_no_tdlib_read_receipt_operation() {
        let implementation = include_str!("telegram.rs");
        let view_messages = ["functions::", "view_", "messages"].concat();
        let open_chat = ["functions::", "open_", "chat"].concat();
        assert!(!implementation.contains(&view_messages));
        assert!(!implementation.contains(&open_chat));
    }
}
