...
fn setup() -> (FloodServer, Project) {
    let store = Store::new(std::env::temp_dir().join(format!("flood-context-test-{}", ulid::Ulid::new()))).unwrap();
...
fn material(server: &FloodServer, project: &Project, kind: ProjectWorkspaceItemKind, content: &str, access: bool) -> ProjectWorkspaceItem {
    server.store.create_project_workspace_item_idempotent(
        &project.id, kind, "Материал", Some("Краткое описание"), content, access,
        &format!("material-{}", ulid::Ulid::new()),
    ).unwrap().value
}

fn change(server: &FloodServer, item: &ProjectWorkspaceItem, content: &str, access: bool) -> ProjectWorkspaceItem {
    server.store.update_project_workspace_item(&item.project_id, &item.id, &item.title,
        item.summary.as_deref(), content, access, &item.version).unwrap()
}

fn brief(server: &FloodServer, project: &Project, budget: Option<usize>) {
    server.get_project_brief(Parameters(ProjectBriefArgs {
        id: project.id.clone(), task_limit: Some(1), context_budget_chars: budget, include_legacy_snapshot: false,
    })).unwrap();
}

fn check(server: &FloodServer, project: &Project) -> ProjectContextCheckOutput {
    server.check_project_context(Parameters(CheckProjectContextArgs { project_id: project.id.clone() })).unwrap().0
}

fn read(server: &FloodServer, item: &ProjectWorkspaceItem, history: bool) -> ProjectWorkspaceItemReadOutput {
    server.get_project_workspace_item(Parameters(GetProjectWorkspaceItemArgs {
        project_id: item.project_id.clone(), id: item.id.clone(), include_history: history,
    })).unwrap().0
}

fn list_args(project: &Project) -> ListProjectWorkspaceItemsArgs {
    ListProjectWorkspaceItemsArgs {
        project_id: project.id.clone(), kind: None, include_content: false, limit: None, cursor: None,
    }
}
...
fn workspace_material_reads_omit_history_without_changing_storage() {
    ...
    let mut item = material(&server, &project, ProjectWorkspaceItemKind::Rule, &"Правило\n".repeat(1000), true);
    for index in 0..8 {
        item = change(&server, &item, &format!("Версия {index}\n{}", "Обязательное правило\n".repeat(1000)), true);
    }
    ...
    eprintln!("Material read payload: current={} bytes, with history={} bytes", current_json.len(), history_json.len());
    assert_eq!(server.store.get_project_workspace_item(&project.id, &item.id).unwrap(), item);
...
fn workspace_reads_enforce_access_for_current_and_historical_content() {
    let (server, project) = setup();
    let hidden = material(&server, &project, ProjectWorkspaceItemKind::Document, "PRIVATE_BODY", false);
    for include_history in [false, true] {
        let error = server.get_project_workspace_item(Parameters(GetProjectWorkspaceItemArgs {
            project_id: project.id.clone(), id: hidden.id.clone(), include_history,
        })).err().expect("disabled material must not be readable");
        assert!(!error.contains("PRIVATE_BODY"));
        assert!(error.contains("не разрешён"));
    }
    let listed = server.list_project_workspace_items(Parameters(list_args(&project))).unwrap().0;
    ...
    assert!(!serde_json::to_string(&history).unwrap().contains("PRIVATE_BODY"));
    assert_eq!(server.store.get_project_workspace_item(&project.id, &hidden.id).unwrap().revisions.len(), 1);
}

#[test]
fn workspace_list_is_compact_paged_and_rejects_stale_or_wrong_scope_cursors() {
    let (server, project) = setup();
    let items = (0..3).map(|_| material(&server, &project, ProjectWorkspaceItemKind::Skill, &"Полный текст\n".repeat(1000), true)).collect::<Vec<_>>();
    let mut args = list_args(&project);
    args.limit = Some(2);
    let first = server.list_project_workspace_items(Parameters(args)).unwrap().0;
    assert_eq!((first.total, first.remaining, first.items.len()), (3, 1, 2));
    assert!(first.items.iter().all(|item| item.content.is_none() && item.content_chars > 10000));
    assert!(!serde_json::to_string(&first).unwrap().contains("Полный текст"));
    let cursor = first.next_cursor.unwrap();
    let mut args = list_args(&project);
    args.cursor = Some(cursor.clone());
    args.include_content = true;
    let second = server.list_project_workspace_items(Parameters(args)).unwrap().0;
    ...
    assert!(server.list_project_workspace_items(Parameters(args)).is_err());
...
fn context_check_is_read_only_and_session_scoped() {
    let (server, project) = setup();
    material(&server, &project, ProjectWorkspaceItemKind::Skill, "Body not needed to check freshness", true);
    assert_eq!(check(&server, &project).status, "missing");
    ...
    assert_eq!(serde_json::to_string(&check(&server, &project)).unwrap(), encoded);
    ...
    assert_eq!(server.store.get_project(&project.id).unwrap().version, project.version);
}

#[test]
fn context_check_reports_material_versions_and_revoked_access_without_acknowledging() {
    let (server, project) = setup();
    let rule = material(&server, &project, ProjectWorkspaceItemKind::Rule, "Старое правило", true);
    let document = material(&server, &project, ProjectWorkspaceItemKind::Document, "Документ", true);
    let revoked = material(&server, &project, ProjectWorkspaceItemKind::Skill, "Больше недоступно", true);
    let hidden = material(&server, &project, ProjectWorkspaceItemKind::Document, "HIDDEN", false);
    brief(&server, &project, None);
    ...
    server.store.delete_project_workspace_item(&project.id, &document.id, &document.version).unwrap();
    change(&server, &revoked, "REVOKED NEW BODY", false);
    let added = material(&server, &project, ProjectWorkspaceItemKind::Skill, "Новый навык", true);
    let result = check(&server, &project);
    ...
    let rule_change = result.changes.iter().find(|change| change.id == rule.id).unwrap();
    assert_eq!(rule_change.change, "updated");
    assert_eq!(rule_change.previous_version.as_deref(), Some(rule.version.as_str()));
    assert_eq!(rule_change.current_version.as_deref(), Some(updated.version.as_str()));
    assert!(result.changes.iter().any(|change| change.id == added.id && change.change == "added"));
    for id in [&document.id, &revoked.id] {
        let removed = result.changes.iter().find(|change| &change.id == id).unwrap();
    ...
    let encoded = serde_json::to_string(&result).unwrap();
    assert!(!encoded.contains("REVOKED") && !encoded.contains("HIDDEN") && !encoded.contains(&hidden.id));
    read(&server, &updated, false);
    assert_eq!(check(&server, &project).status, "stale");
    assert!(server.require_project_context(&project.id).unwrap_err().contains("устарел"));
    assert_eq!(check(&server, &project).previous_context_revision, result.previous_context_revision);
    brief(&server, &project, None);
    assert_eq!(check(&server, &project).status, "current");
}
...
fn context_check_reports_project_changes_without_repeating_memory() {
    let (server, project) = setup();
    brief(&server, &project, None);
    server.store.append_project_memory(&project.id, None, vec!["Решение, которое нужно перечитать".into()]).unwrap();
    let result = check(&server, &project);
    assert_eq!(result.status, "stale");
    assert_eq!(result.project_changed, Some(true));
    assert!(result.changes.is_empty());
    assert!(!serde_json::to_string(&result).unwrap().contains("Решение, которое"));
    assert!(server.require_project_context(&project.id).is_err());
}
...
fn context_check_and_lists_do_not_acknowledge_incomplete_rules() {
    let (server, project) = setup();
    let rule = material(&server, &project, ProjectWorkspaceItemKind::Rule, &"Нужно дочитать\n".repeat(1500), true);
    ...
    for include_content in [false, true] {
        ...
        server.list_project_workspace_items(Parameters(args)).unwrap();
        assert_eq!(check(&server, &project).pending_rule_ids, vec![rule.id.clone()]);
        assert!(server.require_project_context(&project.id).is_err());
    }
...
fn packet_receipt_keeps_the_versions_actually_delivered() {
    for replacement in ["BBBB", "B"] {
        let (server, project) = setup();
        let rule = material(&server, &project, ProjectWorkspaceItemKind::Rule, "AAAA", true);
        let (packet, evidence) = ContextBuilder::new(&server.store)
            .for_project_with_evidence(&project.id, WorkPurpose::Plan, vec![], vec![]).unwrap();
        // Deterministic interleaving: another writer changes the rule after assembly.
        change(&server, &rule, replacement, true);
        server.remember_project_context_for_packet(&packet, &evidence).unwrap();
        let result = check(&server, &project);
        assert_eq!(result.status, "stale");
...
fn known_mutation_keeps_unseen_parallel_changes_stale() {
    let (server, project) = setup();
    let own = material(&server, &project, ProjectWorkspaceItemKind::Document, "Before", true);
    let rule = material(&server, &project, ProjectWorkspaceItemKind::Rule, "Before", true);
    brief(&server, &project, None);
    let result = change(&server, &own, "Own result", true);
    change(&server, &rule, "Unseen change", true);
    server.acknowledge_material_mutation(&result, Some(&own.version)).unwrap();
...
fn preview_args(item: &ProjectWorkspaceItem) -> PreviewProjectWorkspaceItemUpdateArgs {
    PreviewProjectWorkspaceItemUpdateArgs {
        project_id: item.project_id.clone(), id: item.id.clone(),
        title: item.title.clone(), summary: item.summary.clone(),
        content: "Proposed body".into(), agent_access: true,
...
fn workspace_preview_does_not_expose_private_material() {
    let (server, project) = setup();
    let hidden = material(&server, &project, ProjectWorkspaceItemKind::Document, "PRIVATE_CURRENT", false);
    assert!(server.preview_project_workspace_item_update(Parameters(preview_args(&hidden))).is_err());
}

#[test]
fn workspace_preview_does_not_expose_historical_bodies() {
    let (server, project) = setup();
    let hidden = material(&server, &project, ProjectWorkspaceItemKind::Document, "PRIVATE_HISTORY", false);
    let visible = change(&server, &hidden, "Visible", true);
    let preview = server.preview_project_workspace_item_update(Parameters(preview_args(&visible))).unwrap().0;
    assert!(preview.current.revisions.is_empty());
    assert!(!serde_json::to_string(&preview).unwrap().contains("PRIVATE_HISTORY"));
}
