...
Ownership и единый lifecycle Project Context зафиксированы в [`project-context-lifecycle-0.2.0.md`](project-context-lifecycle-0.2.0.md). Эта матрица является контрактом для канонического editor/source-of-truth: новые desktop/MCP surfaces не должны вводить второй независимый mutation path для тех же данных.
...
| Document / Rule / Skill: delete | `delete_project_workspace_item` | `delete_project_workspace_item` | `Store::delete_project_workspace_item` | MCP требует explicit destructive mode, fresh Project Work Context, Agent access и exact version | один version/conflict contract в core |
