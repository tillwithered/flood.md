...
- `ConnectorProjectBinding` proves project/source scope before a read.
- `SourceCatalogRequest` and `TimelineRequest` reject unbounded limits, cursors, and source IDs before provider work starts.
...
- Stale auth, permission denial, rate limiting, invalid requests, unsupported capabilities, and out-of-scope access are distinct normalized failures.
...
6. Run the shared conformance cases for stable identity, project binding, capabilities, cursors, limits, stale auth/rate/permission failures, secret filtering, and disconnected states.
