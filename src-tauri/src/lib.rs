...
use flood_core::{
    ActivityPage, AgentRun, AgentRunPatch, AgentRunState, AgentRunner, AgentRunnerError,
    ...
    let previous_work = task
        ...
        .or_else(|| {
            previous_run
                ...
                .map(|result| bounded(result, 6_000))
        })
...
fn update_run_progress(store: &Store, run_id: &str, line: &str) {
    ...
    if thread_id.is_none() && message.is_none() {
        return;
    }
...
fn finish_work_result(store: &Store, run_id: &str, result: WorkResult) {
    ...
    let memory = if run_state == AgentRunState::ReadyForReview {
        run.as_ref()
            .and_then(|run| {
                store
                    .append_project_memory(&run.project_id, Some(&run.task_id), proposed_memory)
                    .ok()
            })
            .unwrap_or_default()
            .into_iter()
...
enum CodexRunnerMode {
    Start {
        images: Vec<PathBuf>,
        previous_run: Option<Box<AgentRun>>,
    },
    Resume {
        thread_id: String,
        response: String,
...
impl AgentRunner for CodexCliRunner {
    ...
    fn run(&self, packet: WorkPacket) -> Result<WorkResult, AgentRunnerError> {
        let mut command = match &self.mode {
            CodexRunnerMode::Start { .. } => isolated_codex_command(CodexInvocation::Start),
            CodexRunnerMode::Resume { .. } => isolated_codex_command(CodexInvocation::Resume),
        };
        match &self.mode {
            CodexRunnerMode::Start {
                images,
                previous_run,
            } => {
                command
                ...
                command.arg(codex_prompt(&packet, images.len(), previous_run.as_deref()));
            }
            CodexRunnerMode::Resume {
                thread_id,
                response,
            } => {
                let prompt = format!(
...
fn enqueue_codex_task(
    ...
    let schema = serde_json::json!({
        ...
        "required": ["status", "summary", "verification", "remaining", "memory", "blocker", "result"],
    ...
    thread::spawn(move || {
        ...
        let runner = CodexCliRunner {
            ...
            output_path,
            schema_path,
            mode: CodexRunnerMode::Start {
                images,
                previous_run: previous_run.map(Box::new),
            },
        };
        let _provider = runner.provider_id();
        match runner.run(packet) {
            Ok(result) => finish_work_result(&store, &run_id, result),
            Err(_)
                if store
                    .get_agent_run(&run_id)
...
fn enqueue_codex_continuation(
    ...
    let working_directory = PathBuf::from(&run.working_directory);
    if !working_directory.is_dir() {
        return Err("Локальная папка проекта больше недоступна".into());
    }
    let runtime_dir = state
        .store
        .root()
...
fn cancel_agent_run(id: String, state: State<'_, AppState>) -> Result<AgentRun, String> {
    ...
    let pid = state.agent_processes.lock().unwrap().get(&id).copied();
    if let Some(pid) = pid {
        #[cfg(windows)]
...
impl AgentRunner for LocalAutomationRunner {
    fn provider_id(&self) -> &'static str {
        match self.provider {
    ...
    fn run(&self, packet: WorkPacket) -> Result<WorkResult, AgentRunnerError> {
        let prompt = automation_prompt(&packet, &self.claim, &self.candidates, &self.image_labels);
        execute_automation_agent(self.provider, &self.runtime_dir, prompt, &self.images)
...
fn execute_automation_agent(
    ...
    match provider {
        LocalAgentProvider::Codex => execute_automation_codex(runtime_dir, prompt, images),
        LocalAgentProvider::Claude | LocalAgentProvider::Gemini => {
            execute_automation_text_cli(provider, runtime_dir, prompt)
        }
    }
}
...
mod tests {
    ...
    use flood_core::{
        ...
        ContextBuilder, CreateTask, ProjectWorkspaceItemKind, SourceMedia, SourceMediaKind, Store,
        TelegramInboxCandidate, Urgency, WorkAction, WorkDecision, WorkDecisionAction, WorkPacket,
        WorkPurpose, WorkResult, WorkResultStatus,
    };
    ...
    fn telegram_signal_reaches_task_agent_checkpoint_and_result_through_shared_contract() {
        ...
        assert!(
            project
                .memory
                .iter()
                .any(|entry| entry.text == "Форму оплаты проверять на узком экране")
        );
    ...
    fn codex_prompt_resumes_from_latest_checkpoint_before_legacy_run_result() {
        ...
        let packet = ContextBuilder::new(&store)
            ...
            .unwrap();
        let prompt = codex_prompt(&packet, 0, Some(&legacy_run));
